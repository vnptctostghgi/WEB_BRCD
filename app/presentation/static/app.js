...
function ensureMobileGatewayFocusedScriptLoaded() {
  ...
  mobileGatewayFocusedScriptPromise = new Promise((resolve, reject) => {
    ...
    script.src = "/static/mobile-gateway-focused.js?v=20";
    script.defer = true;
...
function ensureInternalEmailScriptLoaded() {
  ...
  internalEmailScriptPromise = new Promise((resolve, reject) => {
    const script = existingScript || document.createElement("script");
    script.src = "/static/internal-email.js?v=8";
    script.defer = true;
...
function ensureFtpMiningScriptLoaded() {
  ...
  ftpMiningScriptPromise = new Promise((resolve, reject) => {
    const script = existingScript || document.createElement("script");
    script.src = "/static/ftp-mining.js?v=8";
    script.defer = true;
...
function ensureReportsRuntimeScriptLoaded() {
  ...
  reportsRuntimeScriptPromise = new Promise((resolve, reject) => {
    const script = existingScript || document.createElement("script");
    script.src = "/static/reports-runtime.js?v=17";
    script.defer = true;
  ...
  return {
    ...
    tabs: Array.isArray(layout?.tabs) ? layout.tabs.map((tab, index) => ({
      ...
      data_sources: Array.isArray(tab.data_sources) ? tab.data_sources.map((source, sourceIndex) => ({
        ...
        row_limit: Number(source.row_limit || 1000),
      })) : [],
...
function collectDashboardBuilderStateFromDom({ strictFilters = false } = {}) {
  ...
  tab.data_sources = [...document.querySelectorAll("#dashboard-data-sources .dashboard-data-source-card")].map((card, index) => {
    ...
    return {
      ...
      row_limit: Number(card.querySelector("[name='source_row_limit']")?.value || 1000),
    };
  ...
  tab.grid_layout = rows.map((row, index) => {
    ...
    const widgets = [...row.querySelectorAll(".builder-widget-card")].map((card) => {
      ...
      const rowFilters = parseDashboardFilters(card.querySelector("[name='row_filters']")?.value || "", strictFilters);
  ...
  const sourceCards = (tab.data_sources || []).map((source, index) => `
    <article class="builder-widget-card dashboard-data-source-card" data-source-index="${index}">
      ...
      <div class="grid gap-2 md:grid-cols-2">
        ...
        <label>Số dòng tối đa<input class="form-control" name="source_row_limit" type="number" min="1" max="20000" value="${Number(source.row_limit || 1000)}" /><small>Toàn bộ biểu đồ dùng chung dữ liệu trong giới hạn này.</small></label>
      </div>
    </article>`).join("");
...
function handleDashboardWorkspaceClick(event) {
  ...
  if (addSourceButton) {
    ...
    tab.data_sources = [...(tab.data_sources || []), { source_code: `SOURCE_${index}`, name: `Nguồn ${index}`, sql_code: "", report_id: null, filters: {}, refresh_interval_seconds: 300, cache_ttl_seconds: 900, row_limit: 1000 }];
    renderDashboardWorkspace();
    return;
  }
...
function handleDashboardWorkspaceChange(event) {
  ...
  const dataSourceSelect = event.target.closest("[name='data_source_code']");
...
function dashboardChartHeight(widgetType, chartData) {
  ...
  const horizontal = widgetType === "percent_bar_chart" || widgetType === "horizontal_multi_bar_chart" || widgetType === "bar_chart" && chartData.orientation === "horizontal";
  ...
  return `
    <div class="sql-report-editor-card" data-onebss-row="${escapeHtml(rowKey)}" data-onebss-report-id="${escapeHtml(report.id || "")}">
      ...
      <div class="onebss-parameter-converter">
        <small class="cell-note onebss-date-hint">Bien ngay ho tro: {{today}}, {today;yyyyMMdd}, {today-7d;dd/MM/yyyy}, {today-3m;dd/MM/yyyy}.</small>
