...
DASHBOARD_WIDGET_TYPES = {"bar_chart", "percent_bar_chart", "percent_column_chart", "pie_chart", "line_chart", "combo_chart", "multi_bar_chart", "horizontal_multi_bar_chart", "multi_line_chart", "data_table", "metric", "data_card", "total_data_card", "google_sheet_embed", "text_title"}
...
def _dynamic_report_sql_worker_state() -> dict[str, Any]:
    ...
    if sql_workers:
        ...
        message = "Chua thay worker SQL online de nhan lenh SQL."
    return {
        "status": status_value,
        "message": message,
...
Goi cai dat co san workstation-install-config.ps1 duoc sinh tu trang quan tri.
...
def workstation_setup_config_script(request: Request) -> str:
    ...
    lines = [
        "# Auto-generated by vnptcto.com for one-click workstation setup.",
        "# Keep this file inside the setup folder; do not send it to people who should not operate the workstation.",
        "$VNPTCTO_WORKSTATION_CONFIG = @{",
    ]
    for key, value, note in config_values:
        lines.append(f"  {key} = {powershell_single_quoted(value)} # {note}")
    lines.extend(
        [
            f"  InstallApiMiddleware = {powershell_bool(True)} # Install local API middleware and Scheduled Tasks.",
            f"  SkipPlaywright = {powershell_bool(False)} # Keep Chromium available for OneBSS automation.",
            f"  StartNow = {powershell_bool(True)} # Start the worker immediately when required secrets are present.",
            "}",
            "",
        ]
    )
...
def workstation_setup_package_bytes(request: Request) -> bytes:
    source_root = Path.cwd()
    stream = BytesIO()
    with ZipFile(stream, "w", ZIP_DEFLATED) as archive:
        archive.writestr(f"{WORKSTATION_SETUP_PACKAGE_ROOT}/README_SETUP.txt", workstation_setup_readme())
        archive.writestr(f"{WORKSTATION_SETUP_PACKAGE_ROOT}/workstation-install-config.ps1", workstation_setup_config_script(request))
