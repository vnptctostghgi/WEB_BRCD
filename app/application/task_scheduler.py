...
class DashboardDatasetScheduler:
    """Queue Oracle refreshes for SQL reports materialized into Supabase tables."""
    ...
    def start(self) -> None:
        if not self.repository: raise RuntimeError("DashboardDatasetScheduler chua duoc configure.")
        if self.thread and self.thread.is_alive(): return
        self.stop_event.clear()
        self.thread = threading.Thread(target=self._loop, name="dashboard-dataset-scheduler", daemon=True)
        self.thread.start()
    ...
    def _loop(self) -> None:
        while not self.stop_event.is_set():
            try: self.queue_due()
            except Exception: logger.exception("Dashboard dataset scheduler failed")
            self.stop_event.wait(self.interval_seconds)
    ...
    def queue_due(self, now: datetime | None = None) -> int:
        ...
        current = now or datetime.now(LOCAL_TIMEZONE)
        count = 0
        for report in self.repository.list_sql_reports():
            if not report.get("is_dashboard_source"): continue
            if str(report.get("dashboard_last_status") or "") in {"queued", "running"}: continue
            last_text = str(report.get("dashboard_last_refresh_at") or "").replace("Z", "+00:00")
            try: last = datetime.fromisoformat(last_text) if last_text else None
...
dashboard_dataset_scheduler = DashboardDatasetScheduler()
