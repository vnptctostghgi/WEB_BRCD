...
class MobileGatewayRepository:
    ...
    def create_pairing_code(self, code_hash: str, created_by: str, ttl_seconds: int) -> dict[str, Any]:
        now = self.now()
        row = {
            "code_hash": code_hash,
        ...
        if self.is_sqlite:
            with self.base.connect() as connection:
                cursor = connection.execute(
                    ...
                    (code_hash, created_by, created_at, expires_at, used_at, used_by_device_id, status)
                    VALUES (?, ?, ?, ?, NULL, NULL, 'active')
                    """,
                    (row["code_hash"], row["created_by"], row["created_at"], row["expires_at"]),
                )
    ...
    def list_pairing_codes(self, limit: int = 20) -> list[dict[str, Any]]:
        if self.is_sqlite:
            return self._sqlite_rows(
                "SELECT id, created_by, created_at, expires_at, used_at, used_by_device_id, status FROM mobile_pairing_codes ORDER BY id DESC LIMIT ?",
                (limit,),
            )
        return self._get(
            "mobile_pairing_codes",
            {"select": "id,created_by,created_at,expires_at,used_at,used_by_device_id,status", "order": "id.desc", "limit": str(limit)},
        )
    ...
    def save_policy(self, device_id: str, payload: DevicePolicyPayload | dict[str, Any], updated_by: str) -> dict[str, Any]:
        ...
        if self.is_sqlite:
            ...
            bool_fields = ["sms_enabled", "notifications_enabled", "clipboard_enabled", "diagnostics_enabled", "force_update"]
            ...
            with self.base.connect() as connection:
                connection.execute(
                    ...
                    (device_id, sms_enabled, notifications_enabled, clipboard_enabled, diagnostics_enabled,
                    ...
                    (
                        ...
                        sqlite_row["clipboard_enabled"], sqlite_row["diagnostics_enabled"], sqlite_row["notification_allowlist"],
    ...
    def decode_policy(self, row: dict[str, Any]) -> dict[str, Any]:
        ...
        for field in ["sms_enabled", "notifications_enabled", "clipboard_enabled", "diagnostics_enabled", "force_update"]:
    ...
    def list_sms(self, *, page: int = 1, page_size: int = 50, device_id: str = "", sender: str = "", query: str = "", otp_only: bool = False) -> dict[str, Any]:
    ...
    def list_notifications(self, limit: int = 100) -> list[dict[str, Any]]:
        if self.is_sqlite:
            rows = self._sqlite_rows("SELECT * FROM mobile_notifications ORDER BY posted_at DESC, id DESC LIMIT ?", (limit,))
        else:
            rows = self._get("mobile_notifications", {"order": "posted_at.desc,id.desc", "limit": str(limit)})
        return [self.decode_notification(row, include_text=True) for row in rows]
