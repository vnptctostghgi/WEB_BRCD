...
delete from public.user_permissions where feature_code in ('admin', 'admin.connections.test', 'auto', 'auto.attt_quarterly', 'auto.attt_links', 'admin.web', 'admin.users', 'admin.connections', 'admin.permissions', 'admin.data_permissions', 'admin.catalogs', 'admin.roles', 'admin.menu', 'admin.work_tasks', 'reports', 'new_reports', 'admin.dashboard_builder', 'vault', 'vault.view', 'vault.manage', 'vault.reveal', 'admin.audit', 'admin.sql_reports', 'admin.onebss_reports', 'report_links', 'shared_report_links');
delete from public.features where code in ('admin', 'admin.connections.test', 'auto', 'auto.attt_quarterly', 'auto.attt_links', 'admin.web', 'admin.users', 'admin.connections', 'admin.permissions', 'admin.data_permissions', 'admin.catalogs', 'admin.roles', 'admin.menu', 'admin.work_tasks', 'reports', 'new_reports', 'admin.dashboard_builder', 'vault', 'vault.view', 'vault.manage', 'vault.reveal', 'admin.audit', 'admin.sql_reports', 'admin.onebss_reports', 'report_links', 'shared_report_links');
insert into public.features (code, name, parent_code, sort_order)
...
insert into public.features (code, name, parent_code, sort_order)
values ('daodulieuftp', 'Đào dữ liệu FTP', 'baocaomoi', 38)
on conflict (code) do update
set name = excluded.name,
    parent_code = excluded.parent_code,
    sort_order = excluded.sort_order;
...
insert into public.features (code, name, parent_code, sort_order)
values ('linkbaocao', 'Link báo cáo', 'baocaomoi', 39)
on conflict (code) do update
set name = excluded.name,
    parent_code = excluded.parent_code,
    sort_order = excluded.sort_order;
...
create table if not exists public.ftp_reports (
  id bigint generated by default as identity primary key,
  ma_bao_cao text not null unique,
  ten_bao_cao text not null,
...
create unique index if not exists ftp_reports_ma_bao_cao_lower_idx
on public.ftp_reports (lower(ma_bao_cao));
...
create table if not exists public.ftp_report_runs (
  run_id text primary key,
  ma_bao_cao text not null,
  ten_bao_cao text not null default '',
  status text not null,
...
grant select, insert, update, delete on public.ftp_reports to service_role;
grant usage, select on sequence public.ftp_reports_id_seq to service_role;
...
grant select, insert, update, delete on public.ftp_report_runs to service_role;
...
begin
  ...
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public'
      and tablename = 'ftp_reports'
      and policyname = 'backend service can manage ftp reports'
  ) then
    create policy "backend service can manage ftp reports"
    on public.ftp_reports
    for all
    to service_role
    using (true)
    with check (true);
  ...
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public'
      and tablename = 'ftp_report_runs'
      and policyname = 'backend service can manage ftp report runs'
  ) then
    create policy "backend service can manage ftp report runs"
    on public.ftp_report_runs
    for all
    to service_role
    using (true)
    with check (true);
